...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y):
        ...
        if y is not None:
            y_emb = self.label_emb(y.to(self.label_emb.weight.device).long())
            emb += torch.squeeze(y_emb)
...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x_t, t, label, clip_denoised=True):
    ...
    @torch.no_grad()
    def p_sample(self, model, x_t, t, label, clip_denoised=True):
        model_mean, _, model_log_variance = self.p_mean_variance(model, x_t, t, label,
    ...
    @torch.no_grad()
    def p_sample_loop(self, model, shape, label):
        batch_size = shape[0]
        device = next(model.parameters()).device
        img = torch.randn(shape, device=device)
        imgs = []
        for i in tqdm(reversed(range(0, timesteps)), desc='sampling loop time step', total=timesteps):
            img = self.p_sample(model, img, torch.full((batch_size,), i, device=device, dtype=torch.long),label)
            imgs.append(img.cpu().numpy())
        return imgs

    @torch.no_grad()
    def sample(self, model, image_size, batch_size=8, channels=3, label=3):
        return self.p_sample_loop(model, shape=(batch_size, channels, image_size, image_size, image_size), label=label)
    ...
    def ddim_sample(
        ...
        sample_img = torch.randn((batch_size, channels, image_size, image_size, image_size), device=device)
        sample_imgs = []
    ...
    def ddim_sample(
        ...
        for i in tqdm(reversed(range(0, ddim_timesteps)), desc='sampling loop time step', total=ddim_timesteps):
            ...
            # 2. predict noise using model
            pred_noise = model(sample_img, t, labels)
...
transform = transforms.Compose([
    ...
    transforms.ToTensor(),  
    transforms.Lambda(lambda x: torch.unsqueeze(x.to(torch.float32), 0)),
...
class CustomDataset(Dataset):
    def __init__(self, mat_file, transform=None):
        ...
        self.labels = np.transpose(np.array(data['labels']))
        data.close()  
...
dataset = CustomDataset('./structure.mat', transform=transform)
...
generated_images = gaussian_diffusion.sample(model, Image_Size, batch_size=1, channels=1, label=0)
img = generated_images[-1].reshape(Image_Size, Image_Size, Image_Size)
data = {'matrix': img}
sio.savemat('output.mat', data)
slice_index = 50  


slice_data = img[:, :, slice_index]
binary_image = ((slice_data.squeeze()+1)*255/2 > 128).astype(int)
